...
net_sizes = [64, 128, 256, 512]

fig, AX = plt.subplots(2,2, figsize=(11,7), sharey=True)
...
for ind, degs in enumerate(k_aver):
    ...
    INS = inset_axes(AX[row, col], width=1.2, height=1.0, loc=4, borderpad=2)
...
for row in range(2):
    for col in range(2):
        AX[row, col].set_ylim([10**-2.3, 10**3.0])
...
# if log-log scale#
AX[0,0].text(70.0, 150.0, r'$\langle k \rangle = 1$', fontsize=24)
AX[0,1].text(70.0, 150.0, r'$\langle k \rangle = 2$', fontsize=24)
AX[1,0].text(70.0, 150.0, r'$\langle k \rangle = 4$', fontsize=24)
AX[1,1].text(70.0, 150.0, r'$\langle k \rangle = 8$', fontsize=24)



AX[0,1].legend(prop={'size':'19'}, bbox_to_anchor=(1.02, 1.02))
