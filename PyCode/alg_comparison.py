...
g = gt.random_graph(50, deg_sampler(), model="erdos", self_loops=True, random=True)
regulation = g.new_edge_property('int')
for n in g.edges():
    regulation[n] = 0
g.edge_properties['regulation'] = regulation  
...
#    vt = timeit.timeit(FFPartitioning(g), number=50)
#else:
#    print("Wrong mode flag.")

time = timeit.timeit(setup=mysetup, stmt=test_code, number=50)
print(time)